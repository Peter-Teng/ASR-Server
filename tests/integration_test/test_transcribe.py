...
def test_transcribe_with_base64(client: TestClient):
    payload = {}
    payload['base64Str'] = wav_to_base64(os.path.join(os.getcwd(), "examples/customer.wav"))
    response = client.post(transcribe_audio, data=json.dumps(payload))
    assert response.status_code == 200
    assert response.json()['code'] == SUCCESS_CODE
    assert response.json()['msg'] == SUCCESS_MSG    
...
def test_denoise_and_transcribe_with_base64(client: TestClient):
    payload = {}
    payload['base64Str'] = wav_to_base64(os.path.join(os.getcwd(), "examples/noisy.wav"))
    response = client.post(denoise_and_transcribe_audio, data=json.dumps(payload))
    assert response.status_code == 200
    assert response.json()['code'] == SUCCESS_CODE
    assert response.json()['msg'] == SUCCESS_MSG 
